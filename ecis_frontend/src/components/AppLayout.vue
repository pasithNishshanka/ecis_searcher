    ...
    <aside class="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200 bg-white lg:flex lg:flex-col">
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <nav class="min-h-0 flex-1 space-y-1 overflow-y-auto p-3 pb-4">
          <RouterLink v-for="item in nav" :key="item.path" :to="item.path" class="nav-item" active-class="nav-active">
    ...
    <div class="lg:pl-64">
      <header
        ...
        <div class="flex items-center gap-3">
          <span class="hidden rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 sm:inline">●
            System online</span>
...
import {
  ClipboardPlus, BedDouble, ClipboardList, Database, FileBarChart2, HeartPulse,
