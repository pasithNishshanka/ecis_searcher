...
<template>
<div>
<PageHeader eyebrow="Inpatient department" title="Wards & Beds" description="Quickly view ward occupancy, bed status and the patient assigned to each bed.">
<div class="flex flex-wrap gap-2">
<BaseButton @click="showWardForm=true"><template #icon><Plus :size="16" /></template>Add ward</BaseButton>
</div>
</PageHeader>
...
<button v-for="w in wards" :key="w.id" @click="scrollToWard(w.id)"
  ...
  <div class="mt-2 h-1.5 rounded-full bg-slate-100">
    <div class="h-1.5 rounded-full bg-teal-600" :style="{width:`${w.capacity ? occ(w)/w.capacity*100 : 0}%`}"></div>
  </div>
</button>
</div>
...
<div><b class="text-emerald-700">{{w.capacity-occ(w)}}</b><span class="ml-1 text-slate-400">available</span></div>
</div>
</div>
<div class="mt-4 h-2 rounded-full bg-slate-200">
<div class="h-2 rounded-full bg-teal-600" :style="{width:`${w.capacity ? occ(w)/w.capacity*100 : 0}%`}"></div>
</div>
</div>

<div class="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3">
<div v-for="b in w.beds" :key="b.id"
...
<span class="badge" :class="b.status==='OCCUPIED'?'bg-blue-100 text-blue-700':b.status==='MAINTENANCE'?'bg-amber-100 text-amber-700':'bg-emerald-100 text-emerald-700'">{{b.status}}</span>
</div>
...
<RouterLink :to="`/patients/${b.patientId}`" class="flex-1"><BaseButton variant="secondary" block size="sm">Patient EHR</BaseButton></RouterLink>
...
<BaseButton block @click="openAssignment(w,b)"><template #icon><UserPlus :size="15" /></template>Assign patient</BaseButton>
</div>
...
<!-- Add ward -->
<Modal :open="showWardForm" title="Add hospital ward" description="Ward and bed capacity are configurable for each hospital." @close="showWardForm=false">
<form @submit.prevent="saveWard" class="grid gap-4 sm:grid-cols-2">
<FormField label="Ward name" required><BaseInput v-model="wf.name" placeholder="Medical Ward B" required /></FormField>
<FormField label="Department" required><BaseInput v-model="wf.department" placeholder="Medicine" required /></FormField>
<FormField label="Floor" required><BaseInput v-model="wf.floor" placeholder="3" required /></FormField>
<FormField label="Bed count" required><BaseInput v-model.number="wf.capacity" type="number" min="1" max="200" required /></FormField>
<div class="sm:col-span-2 flex justify-end"><BaseButton type="submit">Create ward</BaseButton></div>
</form>
</Modal>

<!-- Scalable assignment modal: search, don't load 1000 options -->
<Modal :open="!!assignment" title="Assign patient to bed" description="Search the registered patient by name, patient ID or NIC. No large dropdown is required." @close="assignment=null">
<div v-if="assignment">
...
<FormField class="mt-5" label="Search registered patient"><div class="relative"><BaseInput v-model="patientSearch" class="pr-10" placeholder="Type name, patient ID or NIC..." autofocus /><BaseButton v-if="patientSearch" variant="ghost" size="sm" type="button" class="absolute right-1 top-1 min-h-8 px-2" @click="patientSearch=''">×</BaseButton></div></FormField>
...
<div class="min-w-0 flex-1"><p class="font-bold">{{p.firstName}} {{p.lastName}}</p><p class="text-xs text-slate-400">{{p.patientNumber}} · {{p.nic || 'NIC not recorded'}} · {{p.district}}</p></div>
<span v-if="selectedPatient===p.id" class="font-black text-teal-700">✓</span>
</button>
<div v-if="!patientSearch" class="p-5 text-center text-xs text-slate-400">Start typing to find a patient.</div>
<div v-else-if="!patientMatches.length" class="p-5 text-center text-xs text-slate-400">No registered patient matches "{{patientSearch}}".</div>
</div>
...
<Modal :open="!!patientDetail" title="Current bed patient" description="Quick patient details from the existing EHR." @close="patientDetail=null">
<div v-if="patientDetail" class="space-y-4">
<div class="flex gap-3"><div class="avatar">{{patientDetail.firstName[0]}}{{patientDetail.lastName[0]}}</div><div><h3 class="font-black">{{patientDetail.firstName}} {{patientDetail.lastName}}</h3><p class="text-xs text-slate-400">{{patientDetail.patientNumber}}</p></div></div>
<div class="grid grid-cols-2 gap-2"><Info label="Blood" :value="patientDetail.bloodGroup"/><Info label="Age" :value="`${age(patientDetail.dateOfBirth)} years`"/><Info label="Height" :value="`${patientDetail.heightCm} cm`"/><Info label="District" :value="patientDetail.district"/></div>
<RouterLink :to="`/patients/${patientDetail.id}`"><BaseButton block>Open complete EHR</BaseButton></RouterLink>
</div>
</Modal>
</div>
</template>
