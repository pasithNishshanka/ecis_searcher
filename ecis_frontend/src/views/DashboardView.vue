        ...
        <div class="mt-6 grid gap-6 xl:grid-cols-3">
            <section class="card p-5 xl:col-span-2">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="section-title">Research workflow</h2>
                        <p class="muted mt-1">Clinical data accumulates through normal hospital work.</p>
                    </div>
                </div>
                <div class="mt-6 grid gap-3 md:grid-cols-5">
                    <div v-for="(s, i) in workflow" :key="s" class="rounded-xl border border-slate-200 p-4">
                        <p class="text-xs font-black text-teal-700">0{{ i + 1 }}</p>
                        <p class="mt-2 text-sm font-bold">{{ s }}</p>
                    </div>
                </div>
            </section>
            <section class="card p-5">
                <h2 class="section-title">Emergency queue</h2>
                <div class="mt-4 space-y-3">
                    <div v-for="e in emergencies.slice(0, 4)" :key="e.id" class="rounded-xl bg-slate-50 p-3">
                        <div class="flex justify-between gap-2"><b class="text-sm">{{ e.id }}</b><span class="badge">{{
                                e.status
                                }}</span></div>
                        <p class="mt-1 text-xs text-slate-500">{{ e.description }}</p>
                    </div>
                    <RouterLink to="/emergency">
                        <BaseButton variant="secondary" block>Open emergency</BaseButton>
                    </RouterLink>
                </div>
            </section>
        </div>
